...
class TaskManagerApp:
    def __init__(self, root):
        ...
        ayuda_texto = (
            "✔️ - Da doble clic en una tarea para marcarla o desmarcarla.\n"
            "Escribe una tarea, presiona → Enter para añadirla.\n"
            "✔️ - Selecciona una tarea y presiona C → se marca como completada.\n"
            "🗑️ - Selecciona una tarea y presiona D o Delete → se elimina.\n"
            "Presiona → Escape  para cerrar la app"
        )
        self.info_label = tk.Label(root, text=ayuda_texto, fg="blue", justify="left")
        ...
        self.root.bind("<c>", self.mark_task_completed_shortcut)
        self.root.bind("<C>", self.mark_task_completed_shortcut)  # Mayúscula
    ...
    def mark_task_completed_shortcut(self, event=None):
        """Marca o desmarca una tarea como completada con atajo de teclado"""
        selected_item = self.task_tree.selection()
        if selected_item:
            item_values = self.task_tree.item(selected_item, "values")
            desc = item_values[2]
            new_desc = "✔ " + desc if not desc.startswith("✔ ") else desc[2:]
            self.task_tree.item(selected_item, values=(item_values[0], item_values[1], new_desc))
        else:
            messagebox.showwarning("Advertencia", "Seleccione una tarea para marcar como completada.")

    def delete_task_event(self, event=None):
        """Permite eliminar una tarea con el atajo de teclado"""
        self.delete_task()
